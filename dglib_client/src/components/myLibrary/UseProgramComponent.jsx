...
import { getProgramUseList } from '../../api/programApi';
import { useRecoilValue } from 'recoil';
import { memberIdSelector } from '../../atoms/loginState';
import { useNavigate } from 'react-router-dom';
import dayjs from 'dayjs';
import Loading from '../../routers/Loading';
import { cancelProgram } from '../../api/programApi';
...
const UseProgramComponent = () => {
    ...
    const {
        ...
        queryKey: ['programUses', mid],
        queryFn: () => getProgramUseList(mid),
        enabled: !!mid,
        retry: false,
    });
                                    ...
                                    <tr key={item.progUseNo} className="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td className="py-3 px-2 text-center text-sm">
                                            ...
                                            <span
                                                className={`
                                                    font-semibold
                                                    ${item.current / item.capacity >= 0.8
                                                        ? "text-red-600"
                                                        : "text-blue-600"
                                                    }
                                                `}
                                        ...
                                        <td className="py-3 px-2 text-center">
                                            {item.status === "강의종료" ? (
                                                <span className="text-gray-700 font-semibold text-xs px-2 py-1">
                                                    강의종료
                                                </span>
                                            ) : (
