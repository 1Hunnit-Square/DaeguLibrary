...
const EbookContextMenu = ({active, viewerRef, selection, onAddHighlight, onRemoveHighlight, onUpdateHighlight, onContextmMenuRemove }) => {
    const { highlights } = useRecoilValue(bookLabelState);
    const colorList = [
    ...
    const ColorList = colorList.map((color) => (
        <div name="wrapper" key={color.code} onClick={selection.update ? () => onUpdateHighlight(highlight, color.code) : () => onAddHighlight(color.code)} className="flex items-center justify-center w-full h-[32px] rounded-md cursor-pointer bg-black/0 outline-none transition-all duration-100 ease-[cubic-bezier(0.25,0.1,0.25,1)] hover:bg-[#618eff] focus:bg-[#618eff]" >
