...
const NomalSearchBookComponent = () => {
    ...
    }, [searchURLParams, setSearchParams]);

    const { data, isLoading, isError } = useQuery({
        queryKey: ['librarybooklist', searchURLParams.toString(), searchParams, isChecked],
        queryFn: () => {
            const params = {};
  ...
  return (
    <div>
      <SearchSelectComponent options={searchOption} onSearch={onSearch} input={searchParams.query} defaultCategory={searchParams.option} selectClassName="mr-5" dropdownClassName="w-32" inputClassName="w-100" buttonClassName="right-[calc(20%-100px)]" />
      { isSearched && data.content.length > 1 && <CheckBox checked={isChecked} onChange={onChange} label="결과 내 재검색" checkboxClassName="mt-5 ml-2" />}
