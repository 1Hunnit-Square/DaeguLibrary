...
SILENCE_SECONDS = 2
# VOLUME_THRESHOLD = int(0.005 * 32768)
VOLUME_THRESHOLD = int(0.02 * 32768)
...
async def _transcribe_and_send(writer: asyncio.StreamWriter, audio_buffer: bytearray, client_id: str, mid: str = None):
    ...
    # try:
    #     filename = f"tcp_audio_{int(time.time())}.wav"
    #     logger.info(f"디버깅 오디오 저장: {filename}")
...
async def _receive_client_info(reader: asyncio.StreamReader) -> str:
...
async def tcp_service(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    client_addr = writer.get_extra_info('peername')
    logger.info(f"✅ TCP 클라이언트 연결됨: {client_addr}")
        ...
        while True:
            ...
            if not data:
                ...
                if audio_buffer:
                    ...
                    await _transcribe_and_send(writer, audio_buffer)
                    audio_buffer.clear()
                break
